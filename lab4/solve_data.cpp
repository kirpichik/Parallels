...
void SolveData::initBorders() {
    ...
    for (size_t i = 1; i <= size.y; i++)
        for (size_t j = 1; j <= size.z; j++) {
            ...
            if (borderLower)
                currentArea->set(
                    calculatePhiOnBorder(pos.add(rank * size.x, 0, 0)), pos);
            if (borderUpper)
                currentArea->set(calculatePhiOnBorder(
                                     pos.add((rank + 1) * size.x + 1, 0, 0)),
...
void SolveData::calculateConcurrentBorders() {
    ...
    for (int i = 1; i <= static_cast<int>(size.y); i++)
        for (int j = 1; j <= static_cast<int>(size.z); j++) {
            Point<int> pos(1, i, j);
            if (borderLower == 0)
                nextArea->set(calculateNextPhiAt(pos.add(rank * size.x, 0, 0)),
                              pos);
            if (borderUpper == 0)
                nextArea->set(
                    calculateNextPhiAt(pos.add((rank + 1) * size.x, 0, 0)),
                    pos.add(size.x - 1, 0, 0));
        }
}
...
void SolveData::sendBorders() {
    ...
    if (borderUpper == 0) {
        MPI_Isend(currentArea->getFlatSlice(size.x),
                  (size.y + 2) * (size.z + 2), MPI_DOUBLE, rank + 1, 123,
                  MPI_COMM_WORLD, &sendRequests[1]);
        MPI_Irecv(nextArea->getFlatSlice(size.x + 1),
                  (size.y + 2) * (size.z + 2), MPI_DOUBLE, rank + 1, 123,
...
bool SolveData::needNext() {
    ...
    std::cout << "Max in process[" << rank << "]"  << value  << std::endl;

    MPI_Allgather(&max, 1, MPI_DOUBLE, allMax, 1, MPI_DOUBLE, MPI_COMM_WORLD);
    for (size_t i = 0; i < proc_count; i++) {
        std::cout << "Max value[" << i << "]:" << allMax[i] << std::endl;
        if (max > allMax[i]) {
            max = allMax[i];
        }
    }
    ...
    if (rank == 0) {
        std::cout << "Process:" << rank << " Max absolute in found function "
                  << max << "\n"
                  << std::endl;
    }
